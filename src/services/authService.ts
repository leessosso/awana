...
import * as churchService from './churchService'
...
async function resolveLoginEmail(loginInput: string): Promise<string> {
  ...
  const loginIndexDoc = await getDoc(
    doc(db, 'loginIndex', toLoginIndexKey(normalized)),
  )
...
export async function signIn(email: string, password: string): Promise<User> {
  ...
  try {
    if (!auth) {
      throw new Error(
        'Firebase Auth가 초기화되지 않았습니다. Firebase Console에서 Authentication을 활성화해주세요.',
      )
    }
...
export async function getCurrentUser(): Promise<User | null> {
  ...
  try {
    ...
    const userData = userDoc.data() as User
    return toUserModel(userData, (firebaseUser as any).email)
  } catch (error) {
