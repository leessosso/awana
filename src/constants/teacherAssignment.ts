...
export const TEACHER_PROGRAM_OPTIONS: Array<{
  value: TeacherProgram
  label: string
}> = [
...
export const TEACHER_TEAM_OPTIONS: Array<{
  value: TeacherTeam
  label: string
}> = [
...
export function getTeacherTeamLabel(team?: TeacherTeam): string {
  ...
  return (
    TEACHER_TEAM_OPTIONS.find((item) => item.value === team)?.label ||
    '팀 미지정'
  )
}
...
export function getTeacherProgramLabel(program?: TeacherProgram): string {
  ...
  return (
    TEACHER_PROGRAM_OPTIONS.find((item) => item.value === program)?.label ||
    '클럽 미지정'
  )
}
