...
export async function processStudentData(docId: string, data: any): Promise<Student> {
    ...
    if (tempAssignedTeacherId && tempAssignedUntil && tempAssignedUntil < now) {
      ...
      try {
        ...
        console.log(
          `학생 ${data.name}의 만료된 임시 배정 정보가 정리되었습니다.`,
        )
        // 로컬 변수에서도 정리
...
export async function createStudent(
    ...
    try {
      ...
      if (studentData.tempAssignedUntil) {
        docData.tempAssignedUntil = Timestamp.fromDate(
          studentData.tempAssignedUntil,
        )
      }
    ...
    try {
      ...
      if (studentData.assignedTeacherId !== undefined) {
        updateData.assignedTeacherId =
          studentData.assignedTeacherId || deleteField()
      }
      if ('tempAssignedTeacherId' in studentData) {
        updateData.tempAssignedTeacherId =
          studentData.tempAssignedTeacherId || deleteField()
      }
      if (studentData.birthDate !== undefined) {
        updateData.birthDate = studentData.birthDate
          ? Timestamp.fromDate(studentData.birthDate)
          : deleteField()
      }
      if ('tempAssignedUntil' in studentData) {
        updateData.tempAssignedUntil = studentData.tempAssignedUntil
          ? Timestamp.fromDate(studentData.tempAssignedUntil)
          : deleteField()
      }
...
export async function getStudentsByChurch(
    churchId: string,
    teacherId?: string,
  ): Promise<Student[]> {
    if (!isFirebaseConfigured() || !db) {
    ...
    try {
      const isActiveTempAssignment = (data: Record<string, unknown>) => {
        const tempAssignedTeacherId = data.tempAssignedTeacherId as
          | string
          | undefined
        if (!tempAssignedTeacherId) return false

        const rawTempAssignedUntil = data.tempAssignedUntil as
          | Timestamp
          | Date
          | string
          | undefined
      ...
      if (teacherId) {
        ...
        const assignedPromises = assignedSnapshot.docs.map(async (doc) => {
          ...
          const processedStudent = await processStudentData(doc.id, data)
        ...
        const studentPromises = querySnapshot.docs.map(async (doc) => {
          const data = doc.data()
          return await processStudentData(doc.id, data)
        })
...
export async function getStudentsByClub(churchId: string, club: string): Promise<Student[]> {
    if (!isFirebaseConfigured() || !db) {
      return []
    }
